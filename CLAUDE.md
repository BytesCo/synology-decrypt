...
- **`files.py`** — File-level wrapper: reads encrypted file, writes decrypted output, handles directory creation and cleanup on failure. `decrypt_file` and `decrypt_stream_to_file` write to `<output>.partial` and `os.rename` it onto the final path only after `core.decrypt_stream` returns successfully; cleanup on failure targets the `.partial` path so the final name is never present in a half-written state, even if the process is SIGKILLed. Also exposes `apply_metadata_from_stat` and `apply_metadata_from_zipinfo` helpers used by `--archive` mode; both are non-raising by contract so that metadata failures never turn a successful decrypt into a counted failure.
